...
> **AI agents / coding assistants**: a self-contained reference (install, auth, CLI surface, Python API, workflows, what raises `NotImplementedError`) lives at [`llms.txt`](https://raw.githubusercontent.com/thenervelab/hippius-hub/main/llms.txt) — point your agent at it instead of this README.
...
| `HIPPIUS_UPLOAD_WORKERS` | `8` | Concurrent pack uploads per large file (the per-file thread pool). Effective concurrency for one file is `min(pack count, HIPPIUS_UPLOAD_WORKERS, HIPPIUS_MAX_INFLIGHT_PACKS)` — see [Large-file chunking](#large-file-chunking). File-level fan-out in a folder upload is the `max_workers` argument to `upload_folder`, not this var |
...
Effective pack-upload concurrency for one file is `min(pack count, HIPPIUS_UPLOAD_WORKERS, HIPPIUS_MAX_INFLIGHT_PACKS)`. The in-flight cap defaults to the worker count, so raising `HIPPIUS_MAX_INFLIGHT_PACKS` alone changes nothing for a single file — the per-file pool stays at `HIPPIUS_UPLOAD_WORKERS` (8); raise both to upload more packs at once. Pack count also caps useful workers: a 1 GiB file at the 64 MiB pack size yields 16 packs, so workers beyond 16 sit idle. Each in-flight pack is held in memory (~64 MiB), so peak resident memory scales as in-flight packs × pack size — that memory ceiling is what the cap exists to bound. Past 32 concurrent packs, connections beyond the upload client's 32 idle-per-host pool re-handshake per pack.
