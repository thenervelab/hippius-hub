...
Ported from the equivalent script in `hippius-s3` so both smoke suites produce
the same shape of alert. Parses pytest-json-report output and builds a
human-readable CI alert: which test failed, what the error means in plain
language, and where in *our* code it happened. Suite-level breakage (missing
report, collection error, pytest crash) is reported explicitly so the alert
never claims "no failures" while the workflow is red.
"""
...
SUITE_LABELS = {
    "smoke-production": "production round-trip suite (registry.hippius.com)",
}
...
ERROR_EXPLANATIONS = [
    (
        "ReadTimeout",
        "The registry accepted the connection but did not respond before the client "
        "timeout — it is likely slow, overloaded, or hung.",
    ),
    (
        "ConnectTimeout",
        "Could not establish a connection to the registry in time — it may be down or unreachable.",
    ),
    (
        "ConnectError",
        "The connection to the registry failed or was refused — the service may be down.",
    ),
    (
        "ConnectionError",
        "The connection to the registry failed or was refused — the service may be down.",
    ...
    (
        "SSLError",
        "TLS handshake with the registry failed — check certificates on the target.",
    ),
    (
        "LocalTokenNotFoundError",
        "No usable credential was found — the CI robot secret is missing or was not saved.",
    ),
    (
        "RepositoryNotFoundError",
        "The registry returned 404/401 for the smoke namespace — the repo is gone, or the "
        "CI robot credentials expired and the registry is masking auth failure as not-found.",
    ),
    (
        "EntryNotFoundError",
        "A file that was just uploaded is not resolvable in the manifest — the upload "
        "reported success but the layer annotation or manifest PUT did not take.",
    ),
    (
        "UploadError",
        "The Rust uploader failed mid-transfer — a blob POST/PUT was rejected or the "
        "connection dropped while streaming.",
    ),
    (
        "DownloadError",
        "The Rust downloader failed mid-transfer — a ranged GET was rejected or the "
        "connection dropped while streaming.",
    ),
    (
        "HfHubHTTPError",
        "The registry returned an unexpected HTTP error — see the status code in the traceback.",
    ),
    (
        "AssertionError",
...
def extract_failures(report: Dict[str, Any], report_name: str) -> List[Dict[str, str]]:
    ...
    for test in report["tests"]:
        ...
        # The crash location often points into httpx internals; the frame in our
        # own test file is what a reader actually needs.
...
def build_payload(message: str, run_url: str) -> Dict[str, str]:
    ...
    return {
        "text": (
            f":rotating_light: **hippius-hub production smoke tests failed**\n"
            f"[View run]({run_url})\n\n{message}"
        )
    }
...
def send_webhook(message: str, webhook_url: str, run_url: str):
    """Send alert message to the Mattermost webhook."""
    import subprocess
