...
echo "success bar: upload median >= 80 MiB/s"
echo "download median has no bar yet — record it; it is the number the flip"
echo "changes most (disableredirect proxies every pulled byte through registry)"
echo "endpoint under test: ${ENDPOINT}   <- record this with the result"
