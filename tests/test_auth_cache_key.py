...
def test_token_cache_key_hashes_string_token():
    key = auth._token_cache_key("foo/bar", False, "secret-token")
    repo, push, hashed = key
    assert repo == "foo/bar"
...
def test_token_cache_key_handles_none():
    key = auth._token_cache_key("foo/bar", False, None)
    assert key == ("foo/bar", False, None)
...
def test_token_cache_key_distinguishes_anonymous_from_unset():
    ...
    anon = auth._token_cache_key("foo/bar", False, False)
    unset = auth._token_cache_key("foo/bar", False, None)
    assert anon != unset
